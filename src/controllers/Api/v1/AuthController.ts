...
export default class AuthController {
  ...
  public static async checkResetLink(
    ...
    try {
      ...
      if (!Authorization) {
        ...
        let admin = await UserModel.findById(decoded._id);
        if (admin) {
  ...
  public static async resetPassword(req: Request, res: Response): Promise<any> {
    try {
      ...
      let updateAdmin = await UserModel.findByIdAndUpdate(
        decoded._id,
        {
          $set: {
            password: hashPass,
  ...
  public static async editPassword(req: Request, res: Response): Promise<any> {
    try {
      ...
      const admin: any = await UserModel.findById(id);
      ...
      if (admin) {
        ...
        res
          .status(StatusCodes.BAD_REQUEST)
          // .send(Helper.responseWithoutData(ReasonPhrases.BAD_REQUEST))
          .send(Helper.responseWithoutData("Admin not found in db"));
      }
    } catch (error) {
      res
        .status(StatusCodes.INTERNAL_SERVER_ERROR)
    ...
    try {
      ...
      if (socialAccount) {
        ...
        if (payload.email) {
          ...
          const adminWithSocial = await UserModel.findOne({
            email: payload.email,
          }).populate("socialLogin");
        ...
        const socialAcc = await new userSocialModel({
          socialId: id,
          type: 1,
        }).save();

        const createAdmin = await new UserModel({
          fullname: name,
